...
use crate::{get_color_value, EditorState, ScenePanel, SceneRenderList};

impl ScenePanel {
    
    pub(super) fn render_selection(rndr: &mut malvina::LayerRenderer, client: &Client, editor: &EditorState, render_list: &SceneRenderList) {
        for scene_obj in render_list.objs.iter() {
            match scene_obj {
                SceneObjPtr::Stroke(stroke_ptr) => {
                    ...
                    rndr.render_stroke_selection(&stroke.mesh, get_color_value(&stroke.color, client), editor.scene_obj_transform(*stroke_ptr));
                },
